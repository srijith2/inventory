...
def dashboard():
    if internet_checker.isConnect():
        ...
        data = fetch_query_data.fetch_data(f"SELECT * FROM product_details_{user_id[0]} WHERE status = 'Low stock' OR status = 'Out of stock'")
        if data:
...
def login_validation():
    if internet_checker.isConnect():
        ...
        if (check_query_data.check_two_column(t='user',c1='email',d1=email,c2='password',d2=password)):
            ...
            stock_cost = dashboard_view.stock_cost(email)
            username = check_query_data.check_signle_column('user','email',email)
            data = fetch_query_data.fetch_data(f"SELECT * FROM product_details_{username[0]} WHERE status = 'Low stock' OR status = 'Out of stock' ")
            if data:
...
def sales():
    ...
    if internet_checker.isConnect():
        ...
        if request.method == 'POST':
            ...
            record = fetch_query_data.fetch_data(f"SELECT * FROM SALES_DETAILS_{user_id[0]}")
            ...
            if data==False:
                return render_template("sales_page.html",invalid_msg="Product doesn't exist",user_name_nav=session.get("username"),users=record)
            status = data[8]
            if status  == 'out of stock':
                return render_template("sales_page.html",error="item out of stock",user_name_nav=session.get("username"),users=record)
            ...
            if update == 1 and data:
                ...
                return render_template("sales_page.html",error=f"Low stock only {update} item left",user_name_nav=session.get("username"),users=record)
        user_id = check_query_data.check_signle_column('user','email',admin_email)
        data = fetch_query_data.fetch_data(f"SELECT * FROM SALES_DETAILS_{user_id[0]}")
        if data:
...
def item():
    if internet_checker.isConnect():
        ...
        if request.method == 'POST':
            ...
            status = data[8]
            print(status,type(status),data[8],type(data[8]),"jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj")
            if(low_stock == ""):
                low_stock = data[9]
            print(type(quantity),type(low_stock),"dddddddddddddddddddddddddddddddddddddd")

            update="not updated"

            if(int(quantity) > low_stock):
                update = 'Instock'
            
            elif (int(quantity) <= low_stock and int(quantity) > 1):
                update = 'Low stock'
                send_mail.mail_low_stock(email,product_name,quantity,low_stock)
