...
class send_mail:
    ...
    def mail(email):
        ...
        message = Mail(from_email='vk103688@gmail.com',
            to_emails=email,
        ...
        sg = SendGridAPIClient("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")
...
class dashboard_view:
    ...
    def low_stock(email):
            user_id = check_query_data.check_signle_column('user','email',email)
            sql = f"SELECT count(product_id) FROM product_details_{user_id[0]} WHERE status = 'Low stock' OR status = 'out of stock'"
            stmt = ibm_db.exec_immediate(con,sql)
            data = ibm_db.fetch_tuple(stmt)
            return data[0]
